...
class ManagerTests(unittest.TestCase):
    ...
    def test_config_keeps_devops_out_of_trusted_heavy_and_isolates_billing(self):
        source_path = Path(__file__).parents[1] / "config" / "manager.toml"
        cfg = manager.load_config(source_path)
        routes = manager.admission_routes(cfg)

        trusted = next(route for route in routes if route["name"] == "trusted-heavy")
        billing = next(route for route in routes if route["name"] == "ci-billing-report")
        self.assertNotIn("Tuinstra-DEV/devops", trusted["repositories"])
        self.assertEqual(billing["repositories"], ["Tuinstra-DEV/devops"])
        self.assertEqual(billing["runner_label"], "ops-billing")
        self.assertEqual(
            billing["workflow_paths"],
            [".github/workflows/ci-billing-report.yml"],
        )
        self.assertEqual(billing["events"], ["schedule", "workflow_dispatch"])
        self.assertEqual(billing["branches"], ["main"])
        self.assertEqual(billing["actor_types"], ["User"])

    def test_dedicated_route_requires_fail_closed_identity_filters(self):
        cfg = {
        ...
        with self.assertRaisesRegex(manager.RunnerError, "exact DEV-21 policy"):
            manager.admission_routes(cfg)

    def test_devops_can_never_enter_trusted_heavy(self):
        cfg = {
            "repositories": ["Tuinstra-DEV/gate", "Tuinstra-DEV/devops"],
            "runner_label": "trusted-heavy",
            "runner_group_id": 1,
        }

        with self.assertRaisesRegex(manager.RunnerError, "never be admitted"):
            manager.admission_routes(cfg)

    def test_billing_route_requires_verified_group_three(self):
        cfg = {
            "repositories": ["Tuinstra-DEV/gate"],
            "runner_label": "trusted-heavy",
            "runner_group_id": 1,
    ...
    def test_billing_route_rejects_scope_expansion(self):
        base_route = {
            "name": "ci-billing-report",
            "repositories": ["Tuinstra-DEV/devops"],
            "runner_label": "ops-billing",
        ...
        expansions = (
            {**base_route, "repositories": ["Tuinstra-DEV/devops", "Tuinstra-DEV/gate"]},
            {**base_route, "workflow_paths": [
                ".github/workflows/ci-billing-report.yml",
                ".github/workflows/pr-checks.yml",
            ]},
            {**base_route, "branches": ["main", "develop"]},
            {**base_route, "runner_label": "trusted-heavy"},
        )
        for route in expansions:
            with self.subTest(route=route), self.assertRaisesRegex(
                manager.RunnerError, "exact DEV-21 policy"
            ):
                manager.admission_routes({
                    "repositories": ["Tuinstra-DEV/gate"],
                    "runner_label": "trusted-heavy",
                    "runner_group_id": 1,
                    "dedicated_routes": [route],
                })
    ...
    def test_candidate_jobs_enforces_dedicated_workflow_identity(self):
        client = manager.GitHubClient("secret")
        allowed = {
            "id": 10,
            "workflow_id": 99,
            ...
            "path": ".github/workflows/ci-billing-report.yml",
            "head_branch": "main",
            "head_sha": "a" * 40,
            "repository": {"full_name": "Tuinstra-DEV/devops"},
            "head_repository": {"full_name": "Tuinstra-DEV/devops"},
            "actor": {"login": "marcel-tuinstra", "type": "User"},
            "triggering_actor": {"login": "marcel-tuinstra", "type": "User"},
        }
        blocked = [
            {**allowed, "id": 11, "event": "pull_request"},
            {**allowed, "id": 12, "path": ".github/workflows/pr-checks.yml"},
            {**allowed, "id": 13, "head_branch": "feat/unreviewed"},
            {**allowed, "id": 14, "head_repository": {"full_name": "fork/devops"}},
            {**allowed, "id": 15, "actor": {"login": "dependabot[bot]", "type": "Bot"}},
            {**allowed, "id": 16, "triggering_actor": {"login": "app[bot]", "type": "Bot"}},
            {**allowed, "id": 17,
             "path": ".github/workflows/ci-billing-report.yml@refs/heads/develop"},
        ]
        client.request = mock.Mock(side_effect=[
            {"workflow_runs": [allowed, *blocked]},
            {"jobs": [{"id": 100, "status": "queued",
                       "run_id": 10, "head_sha": "a" * 40,
                       "labels": ["self-hosted", "linux", "x64", "ops-billing"]}]},
            {"workflow_runs": []},
        ])
        policy = {
            "workflow_paths": [".github/workflows/ci-billing-report.yml"],
        ...
        self.assertEqual(
            client.candidate_jobs("Tuinstra-DEV/devops", "ops-billing", policy),
            [{
                "repo": "Tuinstra-DEV/devops", "run_id": 10, "job_id": 100,
                "event": "workflow_dispatch",
                "workflow_id": 99,
                "workflow_path": ".github/workflows/ci-billing-report.yml",
                "head_branch": "main", "head_sha": "a" * 40, "run_attempt": 1,
            }],
        )
        self.assertEqual(client.request.call_count, 3)

    def test_candidate_jobs_accepts_pinned_main_path_and_paginates(self):
        client = manager.GitHubClient("secret")
        allowed = {
            "id": 10,
            "workflow_id": 99,
            "run_attempt": 1,
            "event": "schedule",
            "path": ".github/workflows/ci-billing-report.yml@refs/heads/main",
            "head_branch": "main",
            "head_sha": "b" * 40,
            "repository": {"full_name": "Tuinstra-DEV/devops"},
            "head_repository": {"full_name": "Tuinstra-DEV/devops"},
        ...
        blocked_page = [
            {**allowed, "id": 1000 + index, "path": ".github/workflows/pr-checks.yml"}
            for index in range(100)
        ]
        client.request = mock.Mock(side_effect=[
            {"workflow_runs": blocked_page},
            {"workflow_runs": [allowed]},
            {"jobs": [{
                "id": 100, "run_id": 10, "head_sha": "b" * 40,
                "status": "queued",
                "labels": ["self-hosted", "linux", "x64", "ops-billing"],
            }]},
            {"workflow_runs": []},
        ...
        candidates = client.candidate_jobs("Tuinstra-DEV/devops", "ops-billing", policy)

        self.assertEqual(candidates[0]["workflow_path"],
                         ".github/workflows/ci-billing-report.yml")
        self.assertEqual(candidates[0]["job_id"], 100)
        self.assertIn("page=2", client.request.call_args_list[1].args[1])

    @mock.patch.object(manager, "launch")
    def test_dispatch_uses_shared_capacity_and_route_specific_group(self, launch):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            client = mock.Mock()
            client.candidate_jobs.side_effect = [[{
                "repo": "Tuinstra-DEV/devops", "run_id": 10, "job_id": 20,
                "event": "workflow_dispatch",
                "workflow_id": 99,
                "workflow_path": ".github/workflows/ci-billing-report.yml",
                "head_branch": "main", "head_sha": "a" * 40, "run_attempt": 1,
            ...
            client.generate_jit.assert_called_once_with(
                "Tuinstra-DEV/devops", 20, 3, "ops-billing"
            )
            self.assertEqual(launch.call_args.kwargs["metadata"]["admission_route"],
                             "ci-billing-report")
